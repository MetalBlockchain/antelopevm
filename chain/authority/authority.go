...
var PermissionLevelBillableSize = resource.NewBillableSize(24) ///< over value of weight for safety
...
var KeyWeightBillableSize = resource.NewBillableSize(8) ///< over value of weight for safety, dynamically sizing key
...
var WaitWeightBillableSize = resource.NewBillableSize(16) ///< over value of weight and wait_sec for safety
var AuthorityBillableSize = resource.NewBillableSize(uint64(3*config.FixedOverheadSharedVectorRamBytes) + 4)
...
func (a *Authority) GetBillableSize() uint64 {
	var accountsSize uint64 = uint64(len(a.Accounts)) * PermissionLevelBillableSize
	var waitsSize uint64 = uint64(len(a.Waits)) * WaitWeightBillableSize
	var keysSize uint64 = 0

	for _, key := range a.Keys {
		keysSize += KeyWeightBillableSize
